...
def get_amostra_image(amostra_id: int, db: Session = Depends(get_db)):
    # Usar a função get_amostra_raw para obter o objeto SQLAlchemy
...
# Definir um modelo para receber o status
...
def update_amostra_status(
    amostra_id: int, status_update: StatusUpdate, db: Session = Depends(get_db)
):
    db_amostra = amostra.get_amostra_raw(db, amostra_id)
    if not db_amostra:
        raise HTTPException(status_code=404, detail="Amostra não encontrada")
    ...
    # Retornar o objeto formatado em vez do objeto SQLAlchemy
