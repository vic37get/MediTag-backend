...
def read_amostras(estudo_id: int = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def get_amostra_image(amostra_id: int, image_id: int, db: Session = Depends(get_db)):
...
def reset_amostra(amostra_id: int, db: Session = Depends(get_db)):
...
def list_amostra_images(amostra_id: int, db: Session = Depends(get_db)):
    """
